...
function changeWorkspace(){const select=document.getElementById('workspaceSelect');const next=select?.value;if(!WORKSPACES[next]||next===currentWorkspace){show('stock');return}currentWorkspace=next;localStorage.setItem('mg2_workspace',currentWorkspace);updateWorkspaceUI();show('stock');load()}
